...
if (isset($_POST['btnRegister'])) {
    ...
    if ($count!=0) {
        echo "<script>window.alert('Category $txtcategoryname already exists.')</script>";
        echo "<script>window.location='category_administrator.php'</script>";
    }
    else{
        $query="INSERT INTO category(CategoryName,CategoryStatus) VALUES ('$txtcategoryname','$txtstatus')";
        $result=mysqli_query($connection,$query);
    }
    if ($result) {
        echo "<script>window.alert('Category is added successfully.')</script>";
        echo "<script>window.location='category_administrator.php'</script>";
    }
    else
    {
        echo "<p>Something went wrong in Category Entry!" . mysqli_error($connection) .  "</p>";
    }
}
?>
...
<head>
    ...
    <title>University of Greenwich | Idea Categories</title>
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-content">
                <ul class="lists">
                    ...
                    <li class="list">
                        <a href="category_administrator.php" class="nav_link active">
                        ...
                        <div class="modal-body">
                            <div class="register">
                                <section class="form-container">
                                    ...
                                    <span class="title">Category Entry</span>
                                    <form action="category_administrator.php" method="POST" enctype="multipart/form-data" class="form">
                                        <div class="form-group">
                                            <label for="name">Category Name</label>
                                            <input name="txtcategoryname" type="text" class="form-control" id="name" required>
                                        </div>
                                        <div class="form-group">
                                            <input name="txtstatus" type="hidden" value="0" class="form-control" id="name" required>
                                        </div>
    ...
    <div class="table-container">
                <table class="table">
                    ...
                    <tbody>
                        <?php
                        if ($count < 1) {
                            echo "<tr><p>No Category Found.</p></tr>";
                        }
                        else{
                        ...
                        <td>
                        <?php
                        if ($rows['CategoryStatus']==0) 
                        {
                            echo '<p><a href="categorystatus_administrator.php?CategoryID=' . $rows['CategoryID'] . '&CategoryStatus=1" class="badge badge-success">Active</a></p>';
                        }
                        else
                        {
                            echo '<p><a href="categorystatus_administrator.php?CategoryID=' . $rows['CategoryID'] . '&CategoryStatus=0" class="badge badge-danger">Inactive</a></p>';
                        }
                        ?>
                        </td>
                        <td>
                        <a href="" type="button" data-toggle="modal" data-target="#updateModal<?php echo $rows['CategoryID']?>">Edit</a> | <a href="deletecategory_administrator.php?CategoryID=<?php echo $CategoryID?>">Delete</a>
                        </td>
                        <?php
                            echo "</tr>";
                            include ('updatecategory_administrator.php');
                            }
                        }
                        ?>
                    </tbody>
